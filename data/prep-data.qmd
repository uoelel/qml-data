...
The Morphology and dialectology in the Linguistic Survey of Scotland contains a portion of the morphological questionnaire returns collected for the Linguistic Survey of Scotland (Gaelic).
The data have been transcribed and coded from questionnaire slips by Will Lamb.
...
pupil_width_full <- read_csv("https://osf.io/download/emy8s/")

pupil_width <- pupil_width_full |> 
  group_by(subject, trial) |> 
  mutate(
    pupil.binned = (pupil.binned - mean(pupil.binned)) / sd(pupil.binned)
  ) |> 
  filter(timebins > 1000, timebins < 1500) |> 
  group_by(subject, trial, Condition, Age) |> 
  summarise(
    pupil_max = max(pupil.binned),
    .groups = "drop"
  )

dir.create("data/mclaughlin2023/", showWarnings = FALSE)
write_csv(pupil_width, "data/mclaughlin2023/pupil-width.csv")
```
...
mald_1_1 <- dat |>
  ...
  select(Subject, Item, IsWord, PhonLev, FreqSUBTLEX, RT, ACC) |>
  mutate(
...
ds_files <- list.files("data", pattern = "\\.DS_Store$", recursive = TRUE, full.names = TRUE, all.files = TRUE)
if (length(ds_files) > 0) file.remove(ds_files)
zip("data.zip", "data")
